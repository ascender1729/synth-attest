...
Python, 42 tests) validated in continuous integration on Python 3.11 and 3.12 in two
...
sorted, so that the screening verdict and the order count \textit{within} a batch are not
recoverable from what is anchored. (The number and timing of batches still leak coarse volume; a
fixed-cadence anchoring schedule, noted in Section~\ref{sec:limits}, closes this residual
channel and is future work.) A customer-facing receipt never contains the verdict, preventing the
audit channel from becoming a screening-evasion oracle.
...
validation. The suite is 42 tests; in the no-proprietary-dependency configuration 32 pass and
the 10 backend-specific tests skip, and with the optional backend active (validated source-blind
against the published package) all 42 pass. Both configurations are green in CI on Python 3.11
and 3.12. The suite includes hardening tests for the infohazard guard against obfuscated
sequences, commitment domain separation, and custody pseudonymization.
...
consumes their verdicts as opaque tokens. Commercial entrants such as Aclid (acquired by Ginkgo
Bioworks in 2024) provide first-party sequence screening plus centralized, per-provider customer
screening, with no portable customer-held credential or cross-provider tamper-evident audit.
SecureDNA originated the Exemption Certification System, which issues order-specific, screening-
coupled exemption certificates; synth-attest generalizes that concept into a portable W3C
credential form and adds the portable identity and order-audit layers SecureDNA does not provide.
IBBIS has published customer-screening guidance and forms and plans a 2026 identity landscape
analysis but has not built credential software. The ``Know Your Scientist'' proposal~\cite{kys}
...
identifiers, or a cryptographic audit. Self-sovereign-identity KYC is itself an
established pattern outside biosecurity~\cite{kychain,ssikyc}; our contribution is its first
open application to the DNA-synthesis order chokepoint. Against this landscape, the distinguishing
and best-supported property of synth-attest is being the only \textit{open-source, decentralized
W3C-credential implementation with a tamper-evident order audit}; portability is a feature, not
the core novelty.
...
592 LOC (\texttt{wc -l src/synth\_attest/*.py}); 42 tests collected (\texttt{pytest --co});
32 passed / 10 skipped core and 42 passed with the optional backend (\texttt{pytest});
...
\bibitem{kys} J. Feldman, T. Feldman, and A. I. Anton, ``Know Your Scientist: KYC as Biosecurity
Infrastructure,'' arXiv:2602.06172, 2026.
\bibitem{securedna} C. Baum, J. Berlips, W. Chen, et al. (incl. R. L. Rivest, A. Shamir,
A. C. Yao), ``A system capable of verifiably and privately screening global DNA synthesis,''
arXiv:2403.14023 / bioRxiv 10.1101/2024.03.20.585782, 2024. \url{https://securedna.org/}
...
\bibitem{kychain} C. C. Dragan and M. Manulis, ``KYChain: User-Controlled KYC Data Sharing and
Certification,'' arXiv:2001.01659, 2020.
\bibitem{ssikyc} V. Schlatt, J. Sedlmeir, S. Feulner, and N. Urbach, ``Designing a Framework for
Digital KYC Processes Built on Blockchain-Based Self-Sovereign Identity,'' arXiv:2112.01237, 2021.
\end{thebibliography}
